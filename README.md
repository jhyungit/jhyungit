...
> 🚧 **개발 중** · 게임 5종 구현, 입장 QR·재접속 처리 예정 · 개인 프로젝트

- **휴대폰 센서를 게임 입력으로**: DeviceMotion 가속도 **14m/s²** 임계로 스윙 감지, DeviceOrientation 으로 조준
- **1대 화면 ↔ N대 휴대폰**: 4자리 코드로 페어링. Socket.IO 서버가 게임별 방 5종을 분리해 폰 입력을 중계
- **온라인 1:1 대전 3종**: 호스트 권위 시뮬레이션 + 게스트 추측항법 보간으로 끊김 제거
